  ...
  return (
    <section id="vision" className="py-20 md:py-28 relative z-10 bg-slate-50/60 border-y border-slate-200/60 scroll-mt-16">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 5 Pillars Grid without numbers */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pillars.map((pillar, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`p-7 rounded-2xl bg-white border border-slate-200/80 shadow-2xs hover:shadow-md hover:border-blue-200/80 hover:-translate-y-0.5 transition-all duration-200 group ${isFullOnDesktop}`}
              >
                <h4 className="text-lg font-bold text-slate-900 mb-2.5 group-hover:text-blue-600 transition-colors">
                  {pillar.title}
                </h4>
